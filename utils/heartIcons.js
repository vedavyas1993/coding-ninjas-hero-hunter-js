// empty like icon
export function emptyHeart(id, identifier) {
  return `
    <svg
      ...
      fill="red"
      data-id=${id}
      class="bi bi-heart likes ${identifier}"
      viewBox="0 0 16 16"
    >
...
// filled like icon
export function fillHeart(id, identifier) {
  return `<svg
          xmlns="http://www.w3.org/2000/svg"
          width="22"
          height="22"
...
export function handleLikeAndDislike(hero, identifier) {
  let favouriteHeros = JSON.parse(localStorage.getItem("favouriteHeros"));
  const emptyLikeIcon = emptyHeart(hero.id, identifier);
  const fillLikeIcon = fillHeart(hero.id, identifier);
